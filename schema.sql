...
CREATE TABLE IF NOT EXISTS documents (             -- #pages→documents に変更
    ...
    content     TEXT                               -- 本文（TF-IDFの検索対象メイン）  
);                                                 -- # word_count  INTEGER DEFAULT 0,は必要か？
...
CREATE TABLE IF NOT EXISTS click_logs (
    ...
    FOREIGN KEY (document_id) REFERENCES documents(id) ON DELETE CASCADE #ON DELETE CASCADE を追加して、ドキュメントが削除されたらクリックログも削除されるようにする
);
